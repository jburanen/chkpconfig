...
## .gitignore / .claudeignore maintenance

A general-purpose `.gitignore` exists covering env files, OS/editor cruft, and common Node/Python/Docker artifacts, plus a `.claudeignore` keeping secrets and build noise out of Claude's context. As the actual tech stack is chosen and new tools/frameworks are introduced, update both files to match (e.g. add framework-specific build output, package manager lockfile caches, language-specific artifacts) rather than letting generated files leak into commits or context.
